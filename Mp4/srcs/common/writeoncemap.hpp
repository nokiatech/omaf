...
{
    checkValidation();
    iterator lower = std::lower_bound(begin(), end(), std::make_pair(key, Value()), lessThanProjectFirst);
    if (lower != end() && lower->first == key)
    {
        iterator upper = std::upper_bound(lower, end(), std::make_pair(key, Value()), lessThanProjectFirst);
