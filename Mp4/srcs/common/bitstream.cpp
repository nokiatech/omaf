...
namespace ISOBMFF
{
    ...
    void BitStream::skipBytes(const std::uint64_t x)
    {
        mByteOffset += x;
    }
    ...
    std::uint64_t BitStream::readBoxHeaders(FourCCInt& type)
    {
        auto size = read32Bits();
        type      = read32Bits();
        return size == 1 ? read64Bits() : size;
    }
    ...
    {
        write32Bits(*(std::uint32_t*) &value);
    }

    void BitStream::writeBoxHeaders(FourCCInt type, std::uint64_t boxPayloadSize)
    {
        bool over32BitSize = boxPayloadSize > (UINT32_MAX - 8);
        write32Bits(over32BitSize ? 1 : (uint32_t) boxPayloadSize + 8);
        write32Bits(type.getUInt32());
        if (over32BitSize)
        {
            write64Bits(boxPayloadSize + 16);
        }
    }

    float BitStream::read32BitFloat()
    {
        std::uint32_t value = read32Bits();
        return *(float*) &value;
    }
    ...
    void BitStream::readBoxUUID(Vector<uint8_t>& uuid)
    {
        std::uint64_t initialOffset = getPos();
        std::uint64_t boxSize       = read32Bits();
        FourCCInt boxType           = read32Bits();
        if (boxSize == 1)  // Check if 'largesize' field is used
        {
            boxSize = read64Bits();
        }
