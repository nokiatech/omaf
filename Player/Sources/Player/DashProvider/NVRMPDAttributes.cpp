...
namespace MPDAttribute
{
    ...
    const char_t* kAssociationTypeValueCdsc = "cdsc";
    const char_t* kAssociationTypeValueAudio = "soun";
}  // namespace MPDAttribute
...
StereoRole::Enum DashAttributes::getStereoRole(DashComponents& aDashComponents)
{
    ...
    // Further, the role is a vector of strings, but there is no meaning of having role identifiers in any other place
    // than in the front of the vector
    if (role.size() > 0 &&
        MPDAttribute::ROLE_STEREO_URI.compare(role[0]->GetSchemeIdUri().c_str()) == ComparisonResult::EQUAL)
    {
        if (StringFindFirst(role[0]->GetValue().c_str(), MPDAttribute::ROLE_LEFT) != Npos)
        {
            ...
            int32_t videoChannelIndex =
                atoi(role[0]->GetValue().c_str() +
                     1);  // the string format is ln, where l indicates left, and n indicates the view index
        ...
        else if (StringFindFirst(role[0]->GetValue().c_str(), MPDAttribute::ROLE_RIGHT) != Npos)
        {
            ...
            int32_t videoChannelIndex =
                atoi(role[0]->GetValue().c_str() +
                     1);  // the string format is rn, where r indicates right, and n indicates the view index
...
static SourceDirection::Enum checkFramePacking(const std::vector<dash::xml::INode*>& nodes)
{
    ...
    for (std::vector<dash::xml::INode*>::const_iterator node = nodes.cbegin(); node != nodes.cend(); node++)
    {
        if (MPDAttribute::ESSENTIAL_PROP.compare((*node)->GetName().c_str()) == ComparisonResult::EQUAL)
        {
            ...
            if (!(*node)->GetNodes().empty())
            {
                ...
                for (size_t index = 0; index < subnodes.size() - 1; ++index)
                {
                    if ((StringCompare("schemeIdUri", subnodes[index]->GetName().c_str()) == ComparisonResult::EQUAL) &&
                        (StringCompare(MPDExtension::FRAME_PACKING, subnodes[index]->GetText().c_str()) ==
                        ...
                        (StringCompare("value", subnodes[index + 1]->GetName().c_str()) == ComparisonResult::EQUAL))
                    {
                        ...
                        if (StringCompare(MPDAttribute::FRAME_PACKING_TOP_BOTTOM,
                                          subnodes[index + 1]->GetText().c_str()) == ComparisonResult::EQUAL)
                        {
                            direction = SourceDirection::TOP_BOTTOM;  // we still don't know which one is left channel,
                                                                      // which right
                        }
                        else if (StringCompare(MPDAttribute::FRAME_PACKING_SIDE_BY_SIDE,
                                               subnodes[index + 1]->GetText().c_str()) == ComparisonResult::EQUAL)
                        {
                            direction = SourceDirection::LEFT_RIGHT;  // we still don't know which one is left channel,
                                                                      // which right
                        }
                        else
                        {
                            // not supported, some are not allowed in OMAF, but temporal packing is valid for OMAF but
            ...
            {
                if ((StringCompare(MPDExtension::FRAME_PACKING, (*node)->GetAttributeValue("schemeIdUri").c_str()) ==
                     ComparisonResult::EQUAL))
                {
                    OMAF_LOG_D("Essential info: frame packing %s", (*node)->GetAttributeValue("value").c_str());
                    if (StringCompare(MPDAttribute::FRAME_PACKING_TOP_BOTTOM,
                                      (*node)->GetAttributeValue("value").c_str()) == ComparisonResult::EQUAL)
                    {
                        direction =
                            SourceDirection::TOP_BOTTOM;  // we still don't know which one is left channel, which right
                    }
                    else if (StringCompare(MPDAttribute::FRAME_PACKING_SIDE_BY_SIDE,
                                           (*node)->GetAttributeValue("value").c_str()) == ComparisonResult::EQUAL)
                    {
                        direction =
                            SourceDirection::LEFT_RIGHT;  // we still don't know which one is left channel, which right
                    }
                    else
                    {
                        // not supported, some are not allowed in OMAF, but temporal packing is valid for OMAF but we
