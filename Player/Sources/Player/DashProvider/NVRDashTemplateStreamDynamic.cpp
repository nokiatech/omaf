...
bool_t DashTemplateStreamDynamic::waitForStreamHead()
{
    ...
    if (currentSegmentDelay < minDelaySegments)
    {
        ...
        OMAF_LOG_D(
            "Stream bandwidth %d: Sleeping to increase delay from stream head. Local segment index = %d, server = %d, "
            "diff = %d (%d ms), min delay = %d (%d ms)",
            mBandwidth, mCurrentSegmentIndex, currentServerSideSegmentIndex, currentSegmentDelay,
            currentSegmentDelay * mSegmentDurationInMs, minDelaySegments, minDelaySegments * mSegmentDurationInMs);
        return true;
    }
    else
    {
...
uint32_t DashTemplateStreamDynamic::getSegmentIndexForTime(time_t time, bool_t printLog)
{
    ...
    if (printLog)
    {
        OMAF_LOG_V("getSegmentIDForTime(): Availability start time (seconds since Epoch): %ld",
                   mAvailabilityStartTimeUTCSec);
        OMAF_LOG_V("getSegmentIDForTime(): Requested time = %ld, seconds from start time = %lld", time,
                   secondsFromStartTime);
