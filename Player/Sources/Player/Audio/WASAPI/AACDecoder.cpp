...
{
    AACDecoder::AACDecoder(DecoderInputType inputStreamType,
                           uint32_t inputSampleRate,
                           uint32_t inputChannels,
                           uint32_t outputChannels)
        : mOutputChannels(outputChannels)
        , mSampleBits(16)
        , mAACChannels(inputChannels)
        , mAACSampleRate(inputSampleRate)
    {
        HRESULT hr = S_OK;
        ...
        CHECK_HR_AND_THROW(MFTEnumEx(MFT_CATEGORY_AUDIO_DECODER, flags, &inputFilterAAC, NULL, &activate, &formatCount),
        ...
        CHECK_HR_AND_THROW(mInputType->SetGUID(MF_MT_MAJOR_TYPE, MFMediaType_Audio),
                           "Could not set major type for output media");
        CHECK_HR_AND_THROW(mInputType->SetGUID(MF_MT_SUBTYPE, adtsInput ? MFAudioFormat_ADTS : MFAudioFormat_AAC),
                           "Could not set subtype for output media");
        ...
        CHECK_HR_AND_THROW(mInputType->SetBlob(MF_MT_USER_DATA, userData, sizeof(userData)), "Error input user data.");
    ...
    size_t AACDecoder::getNrAvailablePCMBytes() const
    {
        return mPCMOutputBuffer.size();
    }
    ...
    /**
     * Returns true if adding frame was ok, false if decoder is full and cannot accept data until some of it has been
     * consumed first.
    ...
    bool AACDecoder::tryToPushFrameToDecoder(std::vector<uint8_t> &aacOrAdts)
    {
        ...
        {
            WASAPIImpl::write_message(
                "AACDecoder: Decoder full when trying to add new frame for decode MF_E_NOTACCEPTING");
            return false;
        }
        else if (res != S_OK)
        {
            throw std::exception(
                (std::string("Feeding data to AAC decoder failed.") + std::string(getErrorEnum(res))).c_str());
        }
        else
        {
    ...
    std::vector<uint8_t> AACDecoder::readDecoderOutput()
    {
        ...
        do
        {
            ...
            CHECK_HR_AND_THROW(mAACTransformer->GetOutputStreamInfo(0, &mOutStreamInfo),
                               "Could not read output stream info");
            ...
            else if (res != MF_E_TRANSFORM_NEED_MORE_INPUT)
            {
                ...
                throw std::exception(
                    (std::string("Reading PCM data from decoder failed.") + std::string(getErrorEnum(res))).c_str());
            }
            else
            {
    ...
    void AACDecoder::resetOutputType()
    {
        ...
        do
        {
            CHECK_HR_AND_THROW(mAACTransformer->GetOutputAvailableType(0, i, &mOutputType),
            ...
            WASAPIImpl::write_message(
                "AACDecoder: finding suitable output type (channels: %i sample rate: %i sample depth: %i)",
                typesChannels, typesSampleRate, typesSampleBits);

            if (typesAudioFormat == MFAudioFormat_PCM && typesChannels == mOutputChannels &&
