...
OMAF_NS_BEGIN
    ...
    Error::Enum HeifParser::openInput(const PathName& mediaUri)
    {
        ...
        for (HEIF::FourCC* it = brands.begin(); it != brands.end(); it++)
        {
            ...
            //TODO what other brands to check?
        }
...
// TODO: this can be removed, when new version of HEIF is released and we don't have to use code ported to top of old heif github release
    ...
    Error::Enum HeifParser::selectImage(HEIF::ImageId imageId, MP4VideoStreams& videoStreams) {
        ...
        if (primaryItemType == "grid") {
            ...
            // TODO: if type was identity and GRID (non omaf grid image)... just create flat regions?
            ...
            for (uint32_t row = 0; row < grid.rows; row++) {
                for (uint32_t col = 0; col < grid.columns; col++) {
                    ...
                    createVideoSourceFromImageItem(imageId, videoStreams, sourceInfo, col, row, grid.outputWidth, grid.outputHeight);
                }
            }
        }
        else {
    ...
    Error::Enum HeifParser::readFrame(MP4MediaStream& stream)
    {
        ...
        packet->setDecodingTimeUs(Clock::getMicroseconds());//we use current time as the decoding timestamp; it just indicates the decoding order.
    ...
    void addRegion(
        BasicSourceInfo& sourceInfo, 
        uint32_t inputX, uint32_t inputY,
    ...
    Error::Enum HeifParser::createVideoSourceFromImageItem(HEIF::ImageId imageId, MP4VideoStreams& videoStreams, BasicSourceInfo sourceInfo, uint32_t col, uint32_t row, uint32_t gridWidth, uint32_t gridHeight) {
        HEIF::FourCC codecFourCC;
        mReader->getDecoderCodeType(imageId, codecFourCC);
        HEIF::FeatureBitMask vrFeatures = 0;
        MediaFormat* format = OMAF_NEW(mAllocator, MediaFormat)(MediaFormat::Type::IsVideo, vrFeatures, codecFourCC.value, UNKNOWN_MIME_TYPE);
        ...
        if (gridWidth > 0 && gridHeight > 0) {
            ...
            if (sourceInfo.sourceDirection == SourceDirection::TOP_BOTTOM && projectedTop < 0.5f && projectedBottom > 0.5f) {
                ...
                addRegion(sourceInfo, 0, 0, projectedLeft, projectedTop, projectedRight, 0.5f, currentTileWidth, topTileHeight, gridWidth, gridHeight, width, height);
                addRegion(sourceInfo, 0, bottomInputY, projectedLeft, 0.5f, projectedRight, projectedBottom, currentTileWidth, bottomTileHeight, gridWidth, gridHeight, width, height);
            }
            else if (sourceInfo.sourceDirection == SourceDirection::LEFT_RIGHT && projectedLeft < 0.5f && projectedRight > 0.5f) {
                ...
                addRegion(sourceInfo, 0, 0, projectedLeft, projectedTop, projectedRight, 0.5f, leftTileWidth, currentTileHeight, gridWidth, gridHeight, width, height);
                addRegion(sourceInfo, rightInputX, 0, projectedLeft, 0.5f, projectedRight, projectedBottom, rightTileWidth, currentTileHeight, gridWidth, gridHeight, width, height);
            }
            else {
                // MONO image or source is completely in single stereo region
                addRegion(sourceInfo, 0, 0, projectedLeft, projectedTop, projectedRight, projectedBottom, currentTileWidth, currentTileHeight, gridWidth, gridHeight, width, height);
            }
        }
