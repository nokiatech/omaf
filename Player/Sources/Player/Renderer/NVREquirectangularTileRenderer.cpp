...
void_t EquirectangularTileRenderer::render(const HeadTransform& headTransform,
                                           ...
                                           const RenderingParameters& renderingParameters)
{
    ...
    OMAF_ASSERT(sources.at(0)->type == SourceType::EQUIRECTANGULAR_TILES ||
                    sources.at(0)->type == SourceType::EQUIRECTANGULAR_180,
                "Source is not an equirect panorama with tiles");
    ...
    for (size_t i = 0, endi = sources.getSize(); i < endi; ++i)
    {
        ...
        // frame can consist of e.g 16-24 tiles, and there can be as many directions, so 256-576 elements in the map
        EquirectangularMesh* mesh;
        EquirectTileMeshMap::Iterator viewElement = mGeometryMap.find(source->sourceId);
        ...
        if (viewElement == EquirectTileMeshMap::InvalidIterator)
        {
            ...
            geometry->createTile(makeVector2(source->centerLongitude, source->centerLatitude),
                                 makeVector2(source->spanLongitude, source->spanLatitude), EQUIRECT_MESH_COLS,
                                 EQUIRECT_MESH_ROWS);
