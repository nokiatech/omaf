...
void_t RawRenderer::destroyImpl()
{
...
RawRenderer::Shader::Shader()
    ...
    , viewDirConstant(ShaderConstantID::Invalid)
...
RawRenderer::Shader::~Shader()
{
    ...
    OMAF_ASSERT(viewDirConstant == ShaderConstantID::Invalid, "");

    OMAF_ASSERT(mvpConstant == ShaderConstantID::Invalid, "");
    OMAF_ASSERT(stConstant == ShaderConstantID::Invalid, "");
}

void_t RawRenderer::Shader::create(VideoPixelFormat::Enum textureFormat)
{
    ...
    else if (backendType == RendererType::D3D11 || backendType == RendererType::D3D12)
    {
        const char_t* shader =
            ...
            "\n"
            "   float2 d = uViewDir - input.TexCoord0;\n"
            "   d.y = d.y/2;\n"
            "   float f = clamp(length(d)*100, 0.0f, 1.0f);\n"
            "   float4 red = float4(1.0f,0.0f,0.0f,1.0f);\n"
            "	return (f*rgb + (1-f)*red);\n"
            "}\n";
    ...
    viewDirConstant = RenderBackend::createShaderConstant(handle, "uViewDir", ShaderConstantType::FLOAT2);
    OMAF_ASSERT(viewDirConstant != ShaderConstantID::Invalid, "");
...
void_t RawRenderer::Shader::bind()
{
    RenderBackend::bindShader(handle);
}

void_t RawRenderer::Shader::destroy()
{
    if (isValid())
    {
        ...
        RenderBackend::destroyShaderConstant(viewDirConstant);
        viewDirConstant = ShaderConstantID::Invalid;
...
bool_t RawRenderer::Shader::isValid()
{
    return (handle != ShaderID::Invalid);
}

void_t RawRenderer::Shader::setSourceYConstant(uint32_t textureSampler)
{
    RenderBackend::setShaderConstant(handle, ySourceConstant, &textureSampler);
}

void_t RawRenderer::Shader::setSourceUConstant(uint32_t textureSampler)
{
    RenderBackend::setShaderConstant(handle, uSourceConstant, &textureSampler);
}

void_t RawRenderer::Shader::setSourceVConstant(uint32_t textureSampler)
{
    RenderBackend::setShaderConstant(handle, vSourceConstant, &textureSampler);
}

void_t RawRenderer::Shader::setSourceUVConstant(uint32_t textureSampler)
{
    RenderBackend::setShaderConstant(handle, uvSourceConstant, &textureSampler);
}

void_t RawRenderer::Shader::setSourceConstant(uint32_t textureSampler)
{
    RenderBackend::setShaderConstant(handle, sourceConstant, &textureSampler);
}

void_t RawRenderer::Shader::setTextureRectConstant(const Vector4& textureRect)
{
    RenderBackend::setShaderConstant(handle, textureRectConstant, &textureRect);
}

void_t RawRenderer::Shader::setViewDirConstant(const Vector2& viewDir)
{
    RenderBackend::setShaderConstant(handle, viewDirConstant, &viewDir);
}

void_t RawRenderer::Shader::setMVPConstant(const Matrix44& mvp)
{
    RenderBackend::setShaderConstant(handle, mvpConstant, &mvp);
}

void_t RawRenderer::Shader::setSTConstant(const Matrix44& st)
{
