...
OMAF_NS_BEGIN
    ...
    static TextureFormat::Enum getTextureFormat(RenderTextureFormat::Enum format)
    {
        switch (format)
        {
            case RenderTextureFormat::R8G8B8A8:         return TextureFormat::RGBA8;
            // case RenderTextureFormat::R8G8B8A8_SRGB:    return TextureFormat::RGBA8_SRGB;

            case RenderTextureFormat::D16:              return TextureFormat::D16;
            case RenderTextureFormat::D32F:             return TextureFormat::D32F;
            case RenderTextureFormat::D24S8:            return TextureFormat::D24S8;
            case RenderTextureFormat::D32FS8X24:        return TextureFormat::D32FS8X24;
    ...
    uint32_t RenderingManager::createRenderTarget(const RenderTextureDesc* colorAttachment, const RenderTextureDesc* depthStencilAttachment)
    {
    ...
    void_t RenderingManager::prepareRender(const HeadTransform& headTransform, const RenderingParameters& renderingParamaters)
    {
    ...
    void_t RenderingManager::render(const HeadTransform& headTransform, const RenderSurface& renderSurface, const RenderingParameters& renderingParamaters)
    {
    ...
    void_t RenderingManager::clearFrame(const RenderSurface& renderSurface, const Color4& clearColor)
    {
        ...
        RenderBackend::setViewport(renderSurface.viewport.x, renderSurface.viewport.y, renderSurface.viewport.width, renderSurface.viewport.height);
        RenderBackend::setScissors(renderSurface.viewport.x, renderSurface.viewport.y, renderSurface.viewport.width, renderSurface.viewport.height);

        RenderBackend::clear(ClearMask::COLOR_BUFFER | ClearMask::DEPTH_BUFFER | ClearMask::STENCIL_BUFFER, clearColor.r, clearColor.g, clearColor.b, clearColor.a);

        RenderBackend::popDebugMarker();
    }

    void_t RenderingManager::renderFrame(const HeadTransform& headTransform, const RenderSurface& renderSurface, const RenderingParameters& renderingParameters)
    {
        ...
        else if (renderSurface.eyePosition == EyePosition::AUXILIARY)
        {
            eyePosition = SourcePosition::AUXILIARY;
        }
        else
        {
            eyePosition = SourcePosition::LEFT;
        ...
        for (CoreProviderSources::ConstIterator sit = sources.begin(); sit != sources.end(); ++sit)
        {
            ...
            if (renderingParameters.renderMonoscopic
                && sources.getSize() > 1
            ...
            if (source->sourcePosition == SourcePosition::AUXILIARY
                && eyePosition != SourcePosition::AUXILIARY)
            {
                // Never render an aux stream to something that isn't an aux target
                continue;
            }

            if (eyePosition == SourcePosition::AUXILIARY
                && source->sourcePosition != SourcePosition::AUXILIARY)
            {
                // Never render a non-aux stream to the aux target
                continue;
            }

            if (eyePosition == source->sourcePosition
                ...
                || renderingParameters.renderMonoscopic)
            {
                ...
                if (renderer != OMAF_NULL)
                {
                    renderer->render(headTransform, renderSurface, selectedSources, mRenderMask.getMaskTexture(), renderingParameters);
                }
            }
        }
    ...
    void_t RenderingManager::createDebugTexture(const char_t* filename)
    {
        ...
        textureDesc.numMipMaps = header.numMipmaps;
