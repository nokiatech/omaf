                              ...
                              uint16_t discardMask)
{
    ...
    if (colorBufferFormat != TextureFormat::INVALID)
    {
        ...
        OMAF_GL_CHECK(glTexImage2D(GL_TEXTURE_2D,
                                  0,
        ...
        OMAF_GL_CHECK(glFramebufferTexture2D(GL_FRAMEBUFFER,
    ...
    if (depthStencilBufferFormat != TextureFormat::INVALID)
    {
        ...
        {
            OMAF_GL_CHECK(glFramebufferRenderbuffer(GL_FRAMEBUFFER,
                                                   GL_DEPTH_ATTACHMENT,
                                                   GL_RENDERBUFFER,
                                                   attachment.handle));
        }
    ...
    GLuint status = glCheckFramebufferStatus(GL_FRAMEBUFFER); OMAF_GL_CHECK_ERRORS();
...
bool_t RenderTargetGL::create(TextureGL** attachments,
                              ...
                              uint16_t discardMask)
{
    ...
    for (uint8_t i = 0; i < numAttachments; ++i)
    {
        ...
        else if (!hasDepthComponent && !hasStencilComponent)
        {
            ...
            OMAF_GL_CHECK(glFramebufferTexture2D(GL_FRAMEBUFFER,
                                                GL_COLOR_ATTACHMENT0 + drawBufferIndex,
                                                GL_TEXTURE_2D,
                                                attachment.handle,
                                                0));
...
void_t RenderTargetGL::bind()
{
    ...
    if (mNativeHandle) // TODO: Better handling of native render targets, we need to know its attachments...
    {
        return;
    }
...
void_t RenderTargetGL::destroy()
{
    ...
    if (!mNativeHandle)
    {
        ...
        for (size_t i = 0; i < mNumAttachments; ++i)
        {
            ...
            if (mDestroyAttachments) // TODO: This handle currently in higher level...
            {
                if (attachment.type == AttachmentType::DEPTH_ATTACHMENT ||
                    attachment.type == AttachmentType::STENCIL_ATTACHMENT ||
                    attachment.type == AttachmentType::DEPTH_STENCIL_ATTACHMENT)
                {
                    // TODO: Can be textures....
