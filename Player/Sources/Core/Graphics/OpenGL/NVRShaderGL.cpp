...
static bool_t compileShader(GLuint* handle, GLenum type, const char_t* shader)
{
    ...
    GLuint shaderHandle = glCreateShader(type); OMAF_GL_CHECK_ERRORS();
...
OMAF_INLINE bool_t validateProgram(GLuint handle)
{
    ...
    {
        GLint logLength = 0;
        glGetProgramiv(handle, GL_INFO_LOG_LENGTH, &logLength); OMAF_GL_CHECK_ERRORS();

        char_t buffer[2048];
        glGetProgramInfoLog(handle, logLength, &logLength, buffer); OMAF_GL_CHECK_ERRORS();
...
bool_t ShaderGL::create(const char_t* computeShader)
{
    ...
    mHandle = glCreateProgram(); OMAF_GL_CHECK_ERRORS();
...
bool_t ShaderGL::setUniform(ShaderConstantType::Enum type, HashValue name, const void_t* values, uint32_t numValues)
{
    ...
    if(uniform && uniform->size >= numValues)
    {
        switch (type)
        {
            case ShaderConstantType::BOOL:          return setUniformBool(name, (int32_t*)values, numValues);
            case ShaderConstantType::BOOL2:         return setUniformBool2(name, (int32_t*)values, numValues);
            case ShaderConstantType::BOOL3:         return setUniformBool3(name, (int32_t*)values, numValues);
            case ShaderConstantType::BOOL4:         return setUniformBool4(name, (int32_t*)values, numValues);

            case ShaderConstantType::INTEGER:       return setUniformInteger(name, (int32_t*)values, numValues);
            case ShaderConstantType::INTEGER2:      return setUniformInteger2(name, (int32_t*)values, numValues);
            case ShaderConstantType::INTEGER3:      return setUniformInteger3(name, (int32_t*)values, numValues);
            case ShaderConstantType::INTEGER4:      return setUniformInteger4(name, (int32_t*)values, numValues);

            case ShaderConstantType::FLOAT:         return setUniformFloat(name, (float32_t*)values, numValues);
            case ShaderConstantType::FLOAT2:        return setUniformFloat2(name, (float32_t*)values, numValues);
            case ShaderConstantType::FLOAT3:        return setUniformFloat3(name, (float32_t*)values, numValues);
            case ShaderConstantType::FLOAT4:        return setUniformFloat4(name, (float32_t*)values, numValues);

            case ShaderConstantType::MATRIX22:      return setUniformMatrix22(name, (float32_t*)values, numValues);
            case ShaderConstantType::MATRIX33:      return setUniformMatrix33(name, (float32_t*)values, numValues);
            case ShaderConstantType::MATRIX44:      return setUniformMatrix44(name, (float32_t*)values, numValues);

            case ShaderConstantType::SAMPLER_2D:    return setUniformSampler2D(name, (int32_t*)values, numValues);
                // case ShaderConstantType::SAMPLER_3D:
            case ShaderConstantType::SAMPLER_CUBE:  return setUniformSamplerCube(name, (int32_t*)values, numValues);
...
bool_t ShaderGL::setUniformMatrix22(HashValue name, const float32_t* values, uint32_t numValues)
{
    ...
    if (uniform && uniform->size >= numValues)
    {
        ...
        OMAF_GL_CHECK(glUniformMatrix2fv((GLint)uniform->handle, (GLsizei)numValues, GL_FALSE, (const GLfloat*)values));
...
bool_t ShaderGL::setUniformMatrix33(HashValue name, const float32_t* values, uint32_t numValues)
{
    ...
    if (uniform && uniform->size >= numValues)
    {
        ...
        OMAF_GL_CHECK(glUniformMatrix3fv((GLint)uniform->handle, (GLsizei)numValues, GL_FALSE, (const GLfloat*)values));
...
bool_t ShaderGL::setUniformMatrix44(HashValue name, const float32_t* values, uint32_t numValues)
{
    ...
    if (uniform && uniform->size >= numValues)
    {
        ...
        OMAF_GL_CHECK(glUniformMatrix4fv((GLint)uniform->handle, (GLsizei)numValues, GL_FALSE, (const GLfloat*)values));
...
void_t ShaderGL::queryAttributes(GLuint handle, Attributes& attributes)
{
    ...
    for(GLint i = 0; i < activeAttributes; ++i)
    {
        ...
        GLint location = glGetAttribLocation(handle, nameBuffer); OMAF_GL_CHECK_ERRORS();
...
void_t ShaderGL::queryUniforms(GLuint handle, Uniforms& uniforms)
{
    ...
    for(GLint i = 0; i < activeUniforms; ++i)
    {
        ...
        GLint location = glGetUniformLocation(handle, nameBuffer); OMAF_GL_CHECK_ERRORS();
        OMAF_ASSERT(location != -1, "");
