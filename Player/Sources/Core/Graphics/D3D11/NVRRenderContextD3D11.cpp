...
static const char_t* getFeatureLevelStr(D3D_FEATURE_LEVEL level)
{
    ...
    case D3D_FEATURE_LEVEL_9_1:
        return "Direct3D11 device feature level 9.1";
    case D3D_FEATURE_LEVEL_9_2:
        return "Direct3D11 device feature level 9.2";
    case D3D_FEATURE_LEVEL_9_3:
        return "Direct3D11 device feature level 9.3";

    case D3D_FEATURE_LEVEL_10_0:
        return "Direct3D11 device feature level 10.0";
    case D3D_FEATURE_LEVEL_10_1:
        return "Direct3D11 device feature level 10.1";

    case D3D_FEATURE_LEVEL_11_0:
        return "Direct3D11 device feature level 11.0";
    case D3D_FEATURE_LEVEL_11_1:
        return "Direct3D11 device feature level 11.1";

    /*NOTE: these enums are not defined in older SDK's*/
    case /*D3D_FEATURE_LEVEL_12_0*/ 0xc000:
        return "Direct3D11 device feature level 12.0";
    case /*D3D_FEATURE_LEVEL_12_1*/ 0xc100:
        return "Direct3D11 device feature level 12.1";
...
bool_t RenderContextD3D11::create()
{
    ...
    else
    {
        ...
        HRESULT hr =
            mDevice->CheckFeatureSupport(D3D11_FEATURE(11) /*D3D11_FEATURE_D3D9_SIMPLE_INSTANCING_SUPPORT*/,
                                         &featureData, sizeof(D3D11_FEATURE_DATA_D3D9_SIMPLE_INSTANCING_SUPPORT));
    ...
    if (featureLevel == D3D_FEATURE_LEVEL_10_0 || featureLevel == D3D_FEATURE_LEVEL_10_1)
    {
        ...
        HRESULT hr = mDevice->CheckFeatureSupport(D3D11_FEATURE_D3D10_X_HARDWARE_OPTIONS, &featureData,
    ...
    if (SUCCEEDED(mDevice->QueryInterface(__uuidof(IDXGIDevice), reinterpret_cast<void**>(&dxgiDevice))))
    {
        ...
        if (SUCCEEDED(dxgiDevice->GetAdapter(&adapter)))
        {
            ...
            if (SUCCEEDED(adapter->GetDesc(&desc)))
            {
                ...
                mCapabilities.vendorStr.appendFormat("VendorID: %x, DeviceID: %x, SubSysID: %x, Revision %x",
                                                     desc.VendorId, desc.DeviceId, desc.SubSysId, desc.Revision);
                mCapabilities.deviceStr = cstr;
                mCapabilities.memoryInfo.appendFormat(
                    "%llu (Dedicated Video Memory), %llu (Dedicated System Memory), %llu (Shared System Memory)",
    ...
    if (featureLevel <= D3D_FEATURE_LEVEL_9_2)
    {
        ...
        mCapabilities.maxRenderTargetAttachments =
            min(D3D_FL9_1_SIMULTANEOUS_RENDER_TARGET_COUNT, OMAF_MAX_RENDER_TARGET_ATTACHMENTS);
    }
    else if (featureLevel == D3D_FEATURE_LEVEL_9_3)
    {
        ...
        mCapabilities.maxRenderTargetAttachments =
            min(D3D_FL9_3_SIMULTANEOUS_RENDER_TARGET_COUNT, OMAF_MAX_RENDER_TARGET_ATTACHMENTS);
    }
    else
    {
        ...
        mCapabilities.maxRenderTargetAttachments =
            min(D3D11_SIMULTANEOUS_RENDER_TARGET_COUNT, OMAF_MAX_RENDER_TARGET_ATTACHMENTS);
    }
    ...
    for (size_t f = 0; f < TextureFormat::COUNT; ++f)
    {
        ...
        if (DXGI_FORMAT_UNKNOWN != textureInfo.textureFormat)
        {
            ...
            if (SUCCEEDED(hr))
            {
                mCapabilities.textureFormats[f] = 0 !=
                    (featureData.OutFormatSupport &
                     (D3D11_FORMAT_SUPPORT_TEXTURE2D | D3D11_FORMAT_SUPPORT_TEXTURE3D |
                      D3D11_FORMAT_SUPPORT_TEXTURECUBE |
                      featureData.OutFormatSupport & D3D11_FORMAT_SUPPORT_SHADER_SAMPLE));
            }
        }
    }
...
void_t RenderContextD3D11::createClearStates()
{
    ...
    depthStencilDesc.BackFace.StencilFailOp = depthStencilDesc.BackFace.StencilDepthFailOp =
...
void_t RenderContextD3D11::clippedClear(uint16_t clearMask, float clearColor[], float clearDepth, uint32_t stencil)
{
    ...
    // set our state..
    mQuadState.mShader.setUniform(mDeviceContext, ShaderConstantType::FLOAT4, mQuadState.mClearColor.getHandle(),
                                  clearColor);
...
void_t RenderContextD3D11::clear(uint16_t clearMask, uint32_t color, float32_t depth, int32_t stencil)
{
    ...
    // we draw a quad (or fullviewport triangle) and fill the values in... (IFF scissor/viewport size does not match
    // rendertarget size!
    RenderTargetD3D11& renderTarget = mRenderTargets[mActiveRenderTarget.index];
...
void_t RenderContextD3D11::setShaderConstant(ShaderConstantHandle handle, const void_t* values, uint32_t numValues)
{
    ...
    if (mActiveShader.isValid())
    {
        ...
        bool_t result = shaderProgram.setUniform(mDeviceContext, shaderConstant.getType(), shaderConstant.getHandle(),
                                                 values, numValues);
        OMAF_ASSERT(result, "Shader constant bind failed");
    }
}

void_t RenderContextD3D11::setShaderConstant(ShaderHandle shaderHandle,
...
void_t RenderContextD3D11::drawInstanced(PrimitiveType::Enum primitiveType,
                                         uint32_t offset,
                                         ...
                                         uint32_t instanceCount)
{
    ...
    mDeviceContext->DrawInstanced((UINT) vertexCount, (UINT) instanceCount, (UINT) offset, (UINT) 0);
}

void_t RenderContextD3D11::drawIndexedInstanced(PrimitiveType::Enum primitiveType,
                                                uint32_t offset,
                                                uint32_t indexCount,
                                                uint32_t instanceCount)
{
    ...
    mDeviceContext->DrawIndexedInstanced((UINT) indexCount, (UINT) instanceCount, (UINT) offset, (UINT) 0, (UINT) 0);
}
...
void_t RenderContextD3D11::bindComputeVertexBuffer(uint16_t stage,
...
void_t RenderContextD3D11::bindComputeIndexBuffer(uint16_t stage,
...
void_t
RenderContextD3D11::dispatchCompute(ShaderHandle handle, uint16_t numGroupsX, uint16_t numGroupsY, uint16_t numGroupsZ)
{
    bindShader(handle);
...
bool_t RenderContextD3D11::createVertexBuffer(VertexBufferHandle handle, const VertexBufferDesc& desc)
{
    ...
    return vertexBuffer.create(mDevice, desc.declaration, desc.access, desc.data, desc.size);
}
...
bool_t RenderContextD3D11::createIndexBuffer(IndexBufferHandle handle, const IndexBufferDesc& desc)
{
    ...
    return indexBuffer.create(mDevice, desc.access, desc.data, desc.size);
}
...
bool_t RenderContextD3D11::createShaderConstant(ShaderHandle shaderHandle,
                                                ShaderConstantHandle constantHandle,
                                                const char_t* name,
                                              ...
                                              int16_t discardMask)
{
    ...
    return renderTarget.create(mDevice, mDeviceContext, textures, numAttachments, discardMask);
}
