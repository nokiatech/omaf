...
OMAF_NS_BEGIN
    ...
    bool_t TextureD3D11::createNative(ID3D11Device* device, ID3D11DeviceContext* deviceContext, const NativeTextureDesc& desc)
    {
        OMAF_ASSERT_NOT_NULL(device);
        OMAF_ASSERT_NOT_NULL(deviceContext);
    ...
    void_t TextureD3D11::bindCompute(ID3D11DeviceContext* deviceContext, uint16_t stage, ComputeBufferAccess::Enum computeAccess)
    {
        bool_t needsSRV = false;
        bool_t needsUAV = false;
