...
bool_t ShaderD3D11::create(ID3D11Device* device, const char_t* vertexShader, const char_t* pixelShader)
{
    ...
    result = device->CreateVertexShader(mVertexShaderByteCode->GetBufferPointer(),
                                        mVertexShaderByteCode->GetBufferSize(), NULL, &mVertexShader);
    ...
    result = device->CreatePixelShader(mPixelShaderByteCode->GetBufferPointer(), mPixelShaderByteCode->GetBufferSize(),
                                       NULL, &mPixelShader);
...

    {
        ...
        HRESULT result = D3DReflect(mVertexShaderByteCode->GetBufferPointer(), mVertexShaderByteCode->GetBufferSize(),
        ...
        // Create input layout
        result = device->CreateInputLayout(inputElementDescs, shaderDesc.InputParameters,
                                           mVertexShaderByteCode->GetBufferPointer(),
                                           mVertexShaderByteCode->GetBufferSize(), &mInputLayout);
...
bool_t ShaderD3D11::create(ID3D11Device* device, const char_t* computeShader)
{
    ...
    HRESULT result = device->CreateComputeShader(mComputeShaderByteCode->GetBufferPointer(),
                                                 mComputeShaderByteCode->GetBufferSize(), NULL, &mComputeShader);
...
bool_t ShaderD3D11::setUniform(ID3D11DeviceContext* deviceContext,
                               ...
                               int numValues)
{
    ...
    if (type == ShaderConstantType::SAMPLER_2D || type == ShaderConstantType::SAMPLER_3D ||
    ...
    if (uniform)
    {
        ...
        if (uniform->variableClass == variableClass && uniform->variableType == variableType)
        {
            ...
            size_t writeBytes = typeInfo.sizeInBytes * numValues;
            ...
            {
                OMAF_LOG_W("CONSTANT CACHE SIZE TOO SMALL!");
			}
        }
    }
...
ID3DBlob* ShaderD3D11::compileShader(const char_t* shader, const char_t* entry, const char_t* target)
{
    ...
    HRESULT result =
        D3DCompile(shader, StringByteLength(shader), NULL, NULL, NULL, entry,
