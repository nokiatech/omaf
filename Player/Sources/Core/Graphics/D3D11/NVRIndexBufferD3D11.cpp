...
OMAF_NS_BEGIN
    ...
    void_t IndexBufferD3D11::bind(ID3D11DeviceContext* deviceContext)
    {
        ...
        OMAF_ASSERT_NOT_NULL(device);
        OMAF_ASSERT(device->GetFeatureLevel() > D3D_FEATURE_LEVEL_9_1, "Only DXGI_FORMAT_R32_UINT index buffer format is supported currently");

        // TODO: DXGI_FORMAT format = (device->GetFeatureLevel() == D3D_FEATURE_LEVEL_9_1) ? DXGI_FORMAT_R16_UINT : DXGI_FORMAT_R32_UINT;
    ...
    void_t IndexBufferD3D11::bindCompute(ID3D11DeviceContext* deviceContext, uint16_t stage, ComputeBufferAccess::Enum computeAccess)
    {
