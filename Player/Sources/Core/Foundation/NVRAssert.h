...
#define OMAF_ASSERT(condition, msg, ...)                                                                   \
    ...
    {                                                                                                      \
        if (!(condition))                                                                                  \
            if (OMAF_NS::Assertion::reportFailure(#condition, OMAF_FILE, OMAF_LINE, msg, ##__VA_ARGS__) == \
                OMAF_NS::Assertion::BreakType::HALT)                                                       \
                OMAF_ISSUE_BREAK();                                                                        \
    } while (0)
...
#define OMAF_ASSERT_ALIGNMENT(argument, alignment, offset) \
    OMAF_ASSERT(OMAF_NS::isAligned(argument, alignment, offset), "Argument \"" #argument "\" is not properly aligned")
