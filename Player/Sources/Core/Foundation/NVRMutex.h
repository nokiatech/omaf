...
//       Critical section is lightweight solution for exclusive locking and can be used only by the threads of a single
//       process. More information about this decision can be found here:
//       https://software.intel.com/en-us/articles/choosing-between-synchronization-primitives
//
...
public:
    ...
    // Acquires (locks) OS mutex / critical section. If mutex / critical section is already locked by another thread
    // method blocks thread until mutex / critical section is unlocked.
    void_t lock();

    // Tries to acquire (lock) OS mutex / critical section. If mutex / critical section is already locked by another
    // thread method returns false without locking mutex / critical section.
    bool_t tryLock();
