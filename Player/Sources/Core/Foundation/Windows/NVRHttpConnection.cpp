...
private:
    ...
    static void CALLBACK Callback(_In_ HINTERNET hInternet,
                                  _In_ DWORD_PTR dwContext,
                                  _In_ DWORD dwInternetStatus,
                                  _In_ LPVOID lpvStatusInformation,
                                  _In_ DWORD dwStatusInformationLength);

    void completion(_In_ HINTERNET hInternet,
                    _In_ DWORD dwInternetStatus,
                    _In_ LPVOID lpvStatusInformation,
                    _In_ DWORD dwStatusInformationLength);
...
static void utf8towchar(const utf8_t* data, size_t bytes, wchar_t** res, int* len)
{
    ...
    *len = 0;
    if ((data) && (bytes))
    {
        // well.. according to the test file (http://www.cl.cam.ac.uk/~mgk25/ucs/examples/UTF-8-test.txt) windows,chrome
        // fail... and ofcourse wchar is ucs-16 not utf-16, so we are limited to the first 65535 glyphs.. the same way
        // as this.. then again, technically URI should should use ascii and percent encoding for anything else..
...
void CALLBACK HttpConnectionWIN::Callback(_In_ HINTERNET hInternet,
                                          _In_ DWORD_PTR dwContext,
                                          _In_ DWORD dwInternetStatus,
                                          _In_ LPVOID lpvStatusInformation,
                                          _In_ DWORD dwStatusInformationLength)
{
    ((HttpConnectionWIN*) dwContext)
        ->completion(hInternet, dwInternetStatus, lpvStatusInformation, dwStatusInformationLength);
}

HttpConnectionWIN::HttpConnectionWIN(MemoryAllocator& allocator)
    : mAllocator(allocator)
    , mRequestCompleteEvent(false, false)
    , mSessionClosedEvent(false, false)
    , mConnectClosedEvent(false, false)
    , mHttpDataProcessor(OMAF_NULL)
{
    ...
    hSession = WinHttpOpen(L"" /*User agent needs to be defined!*/,
    ...
    if (hSession == OMAF_NULL)
    {
        ...
        hSession = WinHttpOpen(L"" /*User agent needs to be defined!*/, WINHTTP_ACCESS_TYPE_DEFAULT_PROXY,
                               WINHTTP_NO_PROXY_NAME, WINHTTP_NO_PROXY_BYPASS, WINHTTP_FLAG_ASYNC);
    }
...
bool_t HttpConnectionWIN::setUri(const Url& url)
{
    ...
    bool changed = false;
    if ((mUrlComp.dwHostNameLength != urlComp.dwHostNameLength) || (mUrlComp.nPort != urlComp.nPort) ||
        (mUrlComp.nScheme !=
         urlComp.nScheme))  // if hostname changes or port changes or scheme changes, create a new connection.
    {
        // host changed.
        changed = true;
    }
...
void_t HttpConnectionWIN::setTimeout(uint32_t timeoutMS)
{
    ...
    res = WinHttpSetOption(hSession, WINHTTP_OPTION_CONNECT_TIMEOUT, &timeout,
                           sizeof(timeout));  // max time for connection to be established.
    res = WinHttpSetOption(hSession, WINHTTP_OPTION_RECEIVE_TIMEOUT, &timeout,
                           sizeof(timeout));  // max time each read can take..
    /*WinHttpSetOption(hSession, WINHTTP_OPTION_SEND_TIMEOUT, &timeout, sizeof(timeout));//max time each write can
    take.. WinHttpSetOption(hSession, WINHTTP_OPTION_RECEIVE_RESPONSE_TIMEOUT, &timeout, sizeof(timeout));
    */
    // WinHttpSetTimeouts(hSession, timeoutMS /*dns timeout*/, timeoutMS/*connect timeout*/, timeoutMS/*send timeout*/,
...
static struct
{
    ...
    (DWORD) -1, NULL};
#endif
void HttpConnectionWIN::completion(_In_ HINTERNET hInternet,
                                   _In_ DWORD dwInternetStatus,
                                   _In_ LPVOID lpvStatusInformation,
                                   _In_ DWORD dwStatusInformationLength)
{
    ...
    {
        // failed..
        // WINHTTP_ASYNC_RESULT* res = (WINHTTP_ASYNC_RESULT*)lpvStatusInformation;//we dont actually care why it
        // failed.
    ...
    case WINHTTP_CALLBACK_STATUS_HEADERS_AVAILABLE:
    {
        ...
        ret = WinHttpQueryHeaders(hInternet, WINHTTP_QUERY_STATUS_CODE | WINHTTP_QUERY_FLAG_NUMBER,
                                  WINHTTP_HEADER_NAME_BY_INDEX, &dwStatusCode, &dwSize, WINHTTP_NO_HEADER_INDEX);
        mInternalState.httpStatus = dwStatusCode;
        dwSize = sizeof(dwStatusCode);
        if (WinHttpQueryHeaders(hInternet, WINHTTP_QUERY_CONTENT_LENGTH | WINHTTP_QUERY_FLAG_NUMBER,
                                WINHTTP_HEADER_NAME_BY_INDEX, &dwConLen, &dwSize, WINHTTP_NO_HEADER_INDEX))
        {
            // YAY!, got total length (which might not be correct, if body is compressed, this is the compressed size,
            // but it's still a good guestimates)
            mInternalState.totalBytes = dwConLen;
        }
        dwSize = 0;
        ret = WinHttpQueryHeaders(hInternet, WINHTTP_QUERY_RAW_HEADERS, WINHTTP_HEADER_NAME_BY_INDEX,
                                  WINHTTP_NO_OUTPUT_BUFFER, &dwSize, WINHTTP_NO_HEADER_INDEX);
        if (GetLastError() == ERROR_INSUFFICIENT_BUFFER)
        {
            wchar_t* lpOutBuffer = new wchar_t[dwSize / sizeof(wchar_t)];
            ret = WinHttpQueryHeaders(hInternet, WINHTTP_QUERY_RAW_HEADERS, WINHTTP_HEADER_NAME_BY_INDEX, lpOutBuffer,
        ...
        else
        {
            ...
            // although could be a post where we user expects no return data..... unsure if we MUST read the response
            // body anyway.
            mRequestDuration = Time::getClockTimeUs() - mRequestStartTime;
            OMAF_LOG_V("RequestCompleted:  %lld bytes took: %.4f Ms", mInternalState.totalBytes,
                       mRequestDuration / 1000.f);
    ...
    case WINHTTP_CALLBACK_STATUS_DATA_AVAILABLE:
    {
        ...
        else
        {
            ...
            if (freespace < availdata)
            {
                ...
                OMAF_LOG_V("Reallocating output was:%lld now:%lld", (uint64_t) mOutput->getCapacity(),
                           (uint64_t) mOutput->getCapacity() + need);
...
HttpRequest::Enum HttpConnectionWIN::execute(Method command,
                                             const DataBuffer<uint8_t>* input,
                                             DataBuffer<uint8_t>* output)
{
    mMutex.lock();
    if (hSession == OMAF_NULL)
    {
    ...
    hRequest = WinHttpOpenRequest(hConnect, Verb, mPath, NULL, WINHTTP_NO_REFERER, NULL, flags | WINHTTP_FLAG_REFRESH);
