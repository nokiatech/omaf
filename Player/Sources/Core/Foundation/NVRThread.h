...
class Thread
{
    ...
    public:
        ...
        // Set name of OS thread. Needs to be called before start(), values are stored and applied when the OS thread is started.
        bool_t setName(const char_t* name);

        // Returns stack size of OS thread.
        uint32_t getStackSize() const;

        // Set stack size of OS thread. Needs to be called before start(), values are stored and applied when the OS thread is started.
        bool_t setStackSize(uint32_t stackSize);
    
        // Returns priority of OS thread.
        Priority::Enum getPriority() const;

        // Sets priority of OS thread. Needs to be called before start(), values are stored and applied when the OS thread is started.
